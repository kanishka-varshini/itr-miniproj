...
# task 2: assume a wall location and orientation, develop and implement the task of the robot reaching the wall and then applying a prespecified force in the normal direction of the wall
...
# wall equation: y=-1.5*x+5,  slope=tan(theta)=-1.5,  theta=arctan(-1.5),  normal to wall : arctan(1/)

xd,yd=2,2             # desired position on the wall
q1d,q2d=inv(xd,yd)    # desired angles
x0,y0=3,0             # initial position

Nx=2*np.cos(np.arctan(1/1.5))      # x component of normal force to be applied in Newton
Ny=2*np.sin(np.arctan(1/1.5))      # y component of normal force to be applied in Newton

# torque to be applied after reaching the wall
M=[[-l1*np.sin(q1),l1*np.cos(q1)],[-l2*np.sin(q2),l2*np.cos(q2)]]
N=[Nx,Ny]
[T1f,T2f]=np.matmul(M, N)
print('Extra torque to be applied to get a normal force of 2N after reaching the wall\nat joint 1, T1=,',T1f,'\nat joint 2, T2=',T2f)
